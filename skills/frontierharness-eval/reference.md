...
| `custom` | `--model` (required) | `--secret-name` (required) | `--secret-host` (required) |
...
identified these dependencies. Allowing only the provider and uv hosts leaves
Harbor's registry and verifier dependency installs blocked.
...
`run.json.egress_policy` records `mode: "allowlist"`, `scope: "runtime"`, and the
sorted, deduplicated `allowed_hosts` generated by the same helper used to apply the
policy. `candidate.json` and both reports carry this policy forward. A policy setup
failure prevents agent launch and leaves the trial `infra_invalid`.

The published baselines' applied allowlist was not recorded. Package installation can
change task outcomes, so new runs set `methodology_comparable: false` and remain
unranked even with full task coverage. Run a matched control using the same network
policy and environment before asserting comparability; record the supporting evidence
before changing that flag. A matching pass count alone is insufficient.

Resuming with a different provider host, changed allowlist, or missing policy record
is refused before touching a runtime. Use a new run id for the new policy; retain old
evidence under its original conditions. For a retained runtime that still needs
evidence collection, use the original script version and configuration to recover it.
