...
Clone this repository, open it as the agent's workspace, and work from the repository
root — every path in the skill is relative to it.
...
Two credentials have to be in the environment before you start: `RUNTA_TOKEN` from the
Runta dashboard under Settings -> Runta API Keys, and the provider key for whichever
`<PROVIDER>` you chose (`FIREWORKS_API_KEY` for the default). The prompt tells the agent
to check for them and stop rather than guess.

## The prompt

```text
Read skills/frontierharness-eval/SKILL.md and skills/frontierharness-eval/reference.md
in full before running anything, and follow the skill's workflow. Work from the
repository root. Use FH=skills/frontierharness-eval/scripts for the skill's scripts,
since this repo has its own scripts/ directory.
...
1. Verify prerequisites (runta CLI, RUNTA_TOKEN, the provider key, jq, node >= 18).
   If something is missing, tell me exactly what and stop — do not work around it.
